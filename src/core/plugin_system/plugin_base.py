...
class BasePlugin(abc.ABC):
    ...
    # NEW: Web UI methods that plugins can override
    async def get_web_routes(self) -> List[Dict]:
        """Return list of web routes for this plugin - can be overridden"""
        return []
    
    async def get_static_files(self) -> Dict[str, Path]:
        """Return static file mappings - can be overridden"""
        return {}
    
    async def get_template_variables(self) -> Dict:
        """Return variables for template context - can be overridden"""
        return {}
    
    async def get_ui_assets(self) -> Dict:
        """Return CSS/JS assets to inject - can be overridden"""
        return {
            'css': [],
            'js': [],
            'html_head': ''
    ...
    async def get_plugin_pages(self) -> List[Dict]:
        """Return plugin pages for navigation - can be overridden"""
        return []
